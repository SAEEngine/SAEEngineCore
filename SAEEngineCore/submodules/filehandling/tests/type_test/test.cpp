...
public:
  static void test() {
    for (enum_integral_type n = 0;
         n < (enum_integral_type)FILE_TYPE_E::FILE_TYPE_E_MAX_VALUE; ++n) {
...
int main() {
  ...
  assert(eng::FILE_TYPE{".NOT_A_REAL_FILE_EXTENSION_HAHA"} ==
         eng::FILE_TYPE::BAD_VALUE);
#endif
