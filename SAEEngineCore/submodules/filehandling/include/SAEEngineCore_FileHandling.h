...
protected:
  // This should be set to whatever FILE_TYPE_E::BAD_VALUE is equal to just not
  // directly. The static asserts below will help prevent desync between this
  // value and the true value of FILE_TYPE_E::BAD_VALUE
  constexpr static inline size_t ENUM_MAX_VALUE = 2;
  static_assert(ENUM_MAX_VALUE == FILE_TYPE_E::FILE_TYPE_E_MAX_VALUE,
                "FILE_TYPE::FILE_TYPE_E::FILE_TYPE_E_MAX_VALUE doesn't match "
                "FILE_TYPE::ENUM_MAX_VALUE");
  static_assert(FILE_TYPE_E::BAD_VALUE == FILE_TYPE_E::FILE_TYPE_E_MAX_VALUE,
                "FILE_TYPE::FILE_TYPE_E::BAD_VALUE doesn't match "
                "FILE_TYPE::FILE_TYPE_E::FILE_TYPE_E_MAX_VALUE");
...
public:
  ...
  /**
   * @brief Returns the held FILE_TYPE_E value. This allows FileType to be used
   * in switch statements.
   */
  constexpr operator FILE_TYPE_E() const noexcept { return this->type_; };

#if defined(SAE_ENGINE_CORE_USE_EXCEPTIONS)
  /**
   * @brief Attempts to convert the set FILE_TYPE_E value to a string. Returns
   * nullopt if valid() == false (FILE_TYPE_E == BAD_VALUE).
   * @exception std::invalid_arguement thrown on valid() == false (FILE_TYPE_E
   * == BAD_VALUE).
   */
  ...
  // to_cstring() returns a const char* to avoid a string copy from the
  // optional.

  friend inline constexpr bool operator==(FILE_TYPE _lhs,
                                          FILE_TYPE_E _rhs) noexcept {
  ...
  friend inline constexpr bool operator==(FILE_TYPE_E _lhs,
                                          FILE_TYPE _rhs) noexcept {
    return _rhs == _lhs;
  };
  friend inline constexpr bool operator!=(FILE_TYPE_E _lhs,
                                          FILE_TYPE _rhs) noexcept {
  ...
  /**
   * @brief Converts the provided string to a FILE_TYPE_E value. Will abort on
   * bad string if SAE_ENGINE_CORE_HARD_ERRORS is defined.
   */
  FILE_TYPE(const std::string &_str);

  /**
   * @brief Converts the provided string to a FILE_TYPE_E value. Will set to
   * BAD_VALUE on invalid string.
   */
  ...
  // istream / ostream operator overloading to support standard IO operations
  // (this can be used with lout)

  friend inline std::ostream &operator<<(std::ostream &_ostr,
                                         const FILE_TYPE &_ft) {
  ...
  constexpr FILE_TYPE(FILE_TYPE_E _ft) noexcept : type_{_ft} {};
