...
protected:
  constexpr GrowMode(uint8_t _bits) noexcept : bits_{_bits} {};

private:
  uint8_t bits_ = 0x00;
};

template <typename T> concept cx_arithmetic = std::is_arithmetic_v<T>;

/**
 * @brief Type to wrap up a Z position. Higher value is always closer to the
 * front.
 */
class ZLayer {
public:
...
protected:
  ...
  constexpr static value_type fpoint_to_zlayer(T _v) noexcept {
    return (value_type)(std::clamp(_v, (T)0, std::numeric_limits<T>::max()) *
...
public:
  ...
  /**
   * @brief Converts the z value to a floating point number. This is always >=
   * 0.0f
   * @tparam T floating point type
   * @return The converted floating point value
   */
  template <std::floating_point T = float_t>
  constexpr T to_fpoint() const noexcept {
    return zlayer_to_fpoint<T>(this->layer());
  };
  template <std::floating_point T = float_t>
  explicit constexpr operator T() const noexcept {
    return this->to_fpoint<T>();
  };

  constexpr auto operator<=>(const ZLayer &) const noexcept = default;

  template <cx_arithmetic T> constexpr bool operator<=>(T _rhs) const noexcept {
  ...
  constexpr ZLayer operator+(value_type _i) const noexcept {
    return ZLayer{(value_type)(this->layer() + _i)};
  };
  ...
  constexpr ZLayer(value_type _z) noexcept : z_{_z} {};
  template <std::floating_point T>
  constexpr ZLayer(T _zfloat) noexcept : z_{fpoint_to_zlayer(_zfloat)} {};

  ZLayer &operator=(value_type _z) noexcept {
    this->z_ = _z;
    return *this;
  };
  template <std::floating_point T> ZLayer &operator=(T _z) noexcept {
...
/**
 * @brief Basic object type defining an interface for interacting with a single
 * object.
 */
class GFXObject {
public:
...
/**
 * @brief Base type for handling a group of objects that act as only one
 * GFXObject. Provides protected container functionality.
 */
...
/**
 * @brief Base container type for GFXObjects. Implements public container
 * functionality derived from GFXGroup
 */
...
/**
 * @brief Represents a graphics context - usually a window. This prevents the
...
public:
  ...
  void register_artist(const std::string &_name,
