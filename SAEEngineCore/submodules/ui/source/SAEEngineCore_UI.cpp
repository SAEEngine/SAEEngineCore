...
void UIList::reposition_horizontal() {
  if (this->child_count() != 0) {
    auto _eachWidth =
        (this->bounds().width() - ((this->child_count() - 1) * this->margin_)) /
        (this->child_count());
...
void UIList::reposition_vertical() {
  if (this->child_count() != 0) {
    auto _eachHeight = (this->bounds().height() -
                        ((this->child_count() - 1) * this->margin_)) /
                       (this->child_count());
    this->reposition_vertical(_eachHeight);
  };
};

pixels_t &UIList::margin() noexcept { return this->margin_; };
const pixels_t &UIList::margin() const noexcept { return this->margin_; };

void UIList::set_axis(AXIS _axis) noexcept { this->axis_ = _axis; };
UIList::AXIS UIList::axis() const noexcept { return this->axis_; };

void UIList::set_direction(DIRECTION _dir) noexcept { this->dir_ = _dir; };
UIList::DIRECTION UIList::direction() const noexcept { return this->dir_; };
...
void UIList::use_fixed_size(Rect _r) noexcept { this->fixed_size_ = _r; };
std::optional<Rect> UIList::fixed_size() const noexcept {
  return this->fixed_size_;
};

UIList::UIList(GFXContext *_context, Rect _r, AXIS _axis, DIRECTION _dir,
               pixels_t _margin)
