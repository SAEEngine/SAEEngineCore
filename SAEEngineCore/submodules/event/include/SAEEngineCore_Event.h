...
/**
 * @brief Variant pattern type for storing any type of Event. Uses a variant to
...
public:
  template <EVENT_TYPE_E Type> struct EventType;

  template <> struct EventType<EVENT_TYPE_E::NULL_EVENT> {};
  using evNull = EventType<EVENT_TYPE_E::NULL_EVENT>;

  template <> struct EventType<EVENT_TYPE_E::BLACKBOARD_CHANGE> {
    std::string key;
  };
  using evBlackboardChange = EventType<EVENT_TYPE_E::BLACKBOARD_CHANGE>;

  template <> struct EventType<EVENT_TYPE_E::CURSOR_WINDOW_BOUNDS> {
    enum CURSOR_ACTION { ENTER, EXIT };
    CURSOR_ACTION action;
  };
  using evCursorWindowBounds = EventType<EVENT_TYPE_E::CURSOR_WINDOW_BOUNDS>;

  template <> struct EventType<EVENT_TYPE_E::CURSOR_MOVE> {
    int16_t cursor_x = 0;
    int16_t cursor_y = 0;
  };
  using evCursorMove = EventType<EVENT_TYPE_E::CURSOR_MOVE>;

  template <> struct EventType<EVENT_TYPE_E::MOUSE_EVENT> {
    int button = 0;
    int action = 0;
    int mods = 0;
    int16_t cursor_x = 0;
  ...
  template <> struct EventType<EVENT_TYPE_E::KEY_EVENT> {
    int key = 0;
    int scancode = 0;
    int action = 0;
    int mods = 0;
  ...
  template <> struct EventType<EVENT_TYPE_E::SCROLL_EVENT> {
    double x = 0.0;
    double y = 0.0;
  };
  using evScroll = EventType<EVENT_TYPE_E::SCROLL_EVENT>;

  template <> struct EventType<EVENT_TYPE_E::TEXT_EVENT> {
    unsigned codepoint = 0;
  };
  using evText = EventType<EVENT_TYPE_E::TEXT_EVENT>;

  template <> struct EventType<EVENT_TYPE_E::USER_EVENT> {
    constexpr auto operator<=>(
        const EventType<EVENT_TYPE_E::USER_EVENT> &other) const noexcept {
  ...
  template <> struct EventType<EVENT_TYPE_E::GROW_EVENT> {
    pixels_t dw = 0_px;
    pixels_t dh = 0_px;
  };
  using evGrow = EventType<EVENT_TYPE_E::GROW_EVENT>;

  template <> struct EventType<EVENT_TYPE_E::REFRESH_EVENT> {};
  using evRefresh = EventType<EVENT_TYPE_E::REFRESH_EVENT>;

  template <> struct EventType<EVENT_TYPE_E::WINDOW_CLOSE> {};
...
protected:
  using variant_type = std::variant<
      ...
      EventType<EVENT_TYPE_E::MOUSE_EVENT>, EventType<EVENT_TYPE_E::KEY_EVENT>,
      EventType<EVENT_TYPE_E::SCROLL_EVENT>,
      EventType<EVENT_TYPE_E::TEXT_EVENT>, EventType<EVENT_TYPE_E::USER_EVENT>,
...
public:
  EVENT_TYPE index() const noexcept {
    return EVENT_TYPE{(EVENT_TYPE_E)this->get_variant().index()};
  };
  ...
  template <EVENT_TYPE_E Type> auto &get() {
    return std::get<EventType<Type>>(this->get_variant());
  };
  template <EVENT_TYPE_E Type> const auto &get() const {
    return std::get<EventType<Type>>(this->get_variant());
  };

  Event() : vt_{evNull{}}, broadcast_{false} {};

  template <EVENT_TYPE_E Type>
  Event(const EventType<Type> &_evnt, bool _broadcast = false)
      : vt_{_evnt}, broadcast_{_broadcast} {};
  template <EVENT_TYPE_E Type> Event &operator=(const EventType<Type> &_evnt) {
  ...
  template <EVENT_TYPE_E Type> Event &operator=(EventType<Type> &&_evnt) {
...
public:
  ...
  EventResponse(const Event &_ev) : EventResponse{_ev, nullptr} {};
  EventResponse(Event &&_ev) : EventResponse{std::move(_ev), nullptr} {};
