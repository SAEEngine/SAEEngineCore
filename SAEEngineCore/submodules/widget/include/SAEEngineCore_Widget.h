...
namespace sae::engine::core
{
	struct pixels_t
	{
		...
		friend inline constexpr bool operator==(pixels_t _lhs, pixels_t _rhs) noexcept
		{
		...
		friend inline constexpr pixels_t operator+(pixels_t _lhs, pixels_t _rhs) noexcept
		{
		...
		constexpr explicit operator T() const noexcept { return (T)this->count; };
		...
		template <std::integral T>
		constexpr pixels_t(T _v) noexcept :
			count{ (value_type)_v }
		{};
		template <std::integral T>
		pixels_t& operator=(T _v) noexcept
		{
...
namespace std
{
	constexpr static inline auto midpoint(sae::engine::core::pixels_t _lhs, sae::engine::core::pixels_t _rhs) noexcept
	{
...
{ 
	...
	*/
	...
	public:
		...
		void assign(const Palette& _other)
		{
			this->assign(0, _other);
		};



		Palette(size_type _size) :
			data_(_size)
		{};
		Palette(std::initializer_list<value_type> _colors) :
			data_{ _colors }
		{};
